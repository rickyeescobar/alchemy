...
contract — one line per provider the stack registers, exit code 1 when
...
includes a dotenv file in resolution, and `alchemy provider list` prints
each provider's full contract.
...
- [Profiles](/environments/profiles) — local credential bundles and the CI environment-variable matrix.
