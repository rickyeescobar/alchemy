...
on the CLI. `memory` sets a memory limit (`"512m"`, `"2g"`). If the
container uses more, the kernel stops the container, not the host.
`readOnly: true` mounts the root filesystem read-only.
`noNewPrivileges: true` stops processes from gaining privileges
through setuid binaries.
[Run local services](/docker/local-services) walks through this
composition.
