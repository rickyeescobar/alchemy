...
- A `destroyCommand`-only change — the props are persisted but
  `command` does not re-run (the memoized inputs are unchanged).
- Destroy — delete is a no-op by default; the command's side effects
  are never reversed.
- Destroy with `destroyCommand` set — that command runs instead,
  with the last-deployed `cwd`, `env`, `shell`, and `timeout`; a
  non-zero exit fails the destroy.
