...
explorer — a browsable tree of stacks → stages → resources. This is
the record the next plan diffs against — not the live cloud (see
...
```sh
bun alchemy state get myapp dev_sam Bucket
```
...
```sh
# one stage
bun alchemy state clear myapp pr-42 --yes

# a whole stack
bun alchemy state clear myapp --yes
```
...
bun alchemy state clear --backend local
```
