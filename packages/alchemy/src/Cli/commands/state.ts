...
const pathArgument = Argument.string("path").pipe(
  Argument.withDescription("State path (stack/stage/namespace/resource.json)"),
  Argument.optional,
);
...
const allowWriteOperation = Flag.boolean("allow-write-operation").pipe(
  Flag.withDescription("Allow destructive state-store operations"),
  Flag.withDefault(false),
);
...
const deleteCommand = Command.make(
  ...
  instrumentCommand("state.delete")(
    Effect.fn(function* ({ path, recursive, allowWriteOperation, ...rest }) {
      if (!allowWriteOperation) {
        return yield* Effect.fail(
          new UserInputError({
            message: "state deletion requires --allow-write-operation",
          }),
        );
      }
        ...
        Effect.gen(function* () {
          ...
          if (parts.length === 1) {
            ...
            if (resources.length === 0) {
              return yield* Effect.fail(
                new UserInputError({
                  message: "output.json cannot be deleted independently",
                }),
              );
            }
...
const stateExplorer = (
  args: StateArgs & { readonly allowWriteOperation: boolean },
) =>
  withStateService(args, (state) =>
    Effect.gen(function* () {
      ...
      const source: StateExplorerSource = {
        backend: state.id,
        writable: args.allowWriteOperation,
        listStacks: () => Effect.runPromise(state.listStacks()),
        listStages: (stack) => Effect.runPromise(state.listStages(stack)),
        listResources: (stack, stage) =>
          Effect.runPromise(state.list({ stack, stage })),
        readFile: (file) =>
          Effect.runPromise(
            file.kind === "output" ? state.getOutput(file) : state.get(file),
          ).then(encodeState),
        deleteNodes: (nodes) =>
          Effect.runPromise(
            Effect.gen(function* () {
              ...
              yield* Effect.forEach(
                ...
                (node) => {
                  ...
                  if (node.kind === "output") {
                    return Effect.die(
                      new Error("output.json cannot be deleted independently"),
                    );
                  }
                  ...
                  > = [];
                  const collect = (
                    children: ReadonlyArray<StateBrowserNode>,
                  ) => {
...
export const stateCommand = Command.make(
  "state",
  { main: config, envFile, profile, backend, allowWriteOperation },
