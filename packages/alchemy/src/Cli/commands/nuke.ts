  ...
  instrumentCommand(
    ...
    Effect.fn(function* ({
      ...
      if (
        ...
        const chosen = yield* (yield* CliKit.CliKit).prompt.multiSelect({
      ...
      if (targets.length === 0) {
        yield* (yield* CliKit.CliKit).output.info("Nothing to delete.");
        return;
      }
      ...
      if (remaining.length > 0) {
        yield* (yield* CliKit.CliKit).output.error(
