 ...
 */
import { Box, Text, useCliEnvironment } from "../CliKit/components.ts";
...
export const Logo = ({ cols }: { cols: number }): JSX.Element | null => {
  const { unicode } = useCliEnvironment();
  const lines = useMemo(
    () => (unicode ? rasterizeLogo(cols) : []),
    [cols, unicode],
  );
  // Braille rasterization is mojibake on non-Unicode terminals.
